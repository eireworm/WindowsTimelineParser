...
def get_activities_cache_db_absolute_path():
    print("[1] Specifiy a custom fully qualified path to ActivitiesCache.db")
    print("[2] Try find this file automatically")
    op_1 = None
    hasChosen = False
    while not hasChosen:
        try:
            op_1 = int(input("Choose where to find the database: "))
            if op_1 == 1 or op_1 == 2:
                break
            else:
    ...
    if op_1 == 1:
        ...
        if os_path.isfile(path):
            ...
            print("This file you entered does not exist.")
    elif op_1 == 2:
...
def app_usage_report(con):
    ...
    for row in rows:
        ...
        app_name = str(json.loads(row[0])[0]["application"]) \
            ...
            .replace('Microsoft.AutoGenerated.{923DD477-5846-686B-A659-0FCCD73851A8}', "Microsoft.Windows.Shell.RunDialog", 1)
...
def app_launch_times_and_params_report(con):
    cur = con.cursor()
    cur.execute("SELECT AppId, StartTime, Payload, ActivityType, `Group` FROM ActivityOperation")
    rows = cur.fetchall()
    csv_data = "StartTime,ActivityType,AppName,DisplayName,DisplayText,description,activeDurationSeconds,TimeZone,clipboardDataId"
    for row in rows:
        app_name = str(json.loads(row[0])[0]["application"]) \
            .replace('{6D809377-6AF0-444B-8957-A3773F02200E}', "C:\Program Files", 1) \
            .replace('{7C5A40EF-A0FB-4BFC-874A-C0F2E0B9FA8E}', "C:\Program Files (x86)", 1) \
        ...
        if activity_type == 5:
            activity_type = "Open App/File"
        if activity_type == 6:
            activity_type = "Change focus"
        if activity_type == 16:
            activity_type = str(row[4]) + " to/from app"
        if activity_type != 12 and activity_type != 11:
            payload = json.loads(payload)
            try: display_name = payload["appDisplayName"] 
            except: display_name = ''
            try: display_text = payload["displayText"] 
            except: display_text = ''
            try: p_description = payload["description"] 
            except: p_description = ''
            try: time_zone = payload["userTimezone"] 
            except: time_zone = '' 
            try: active_duration_seconds = payload["activeDurationSeconds"] 
            except: active_duration_seconds = ''
            try: clipboard_data_id = payload["clipboardDataId"] 
            except: clipboard_data_id = ''
            csv_data += "\n" + start_time + "," + str(activity_type)  + "," + app_name + "," + display_name + "," + \
                display_text + "," + p_description+ "," + str(active_duration_seconds) + "," + time_zone + "," + \
                    clipboard_data_id
